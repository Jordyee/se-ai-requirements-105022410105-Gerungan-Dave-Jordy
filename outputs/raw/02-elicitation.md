...
This file is the first AI-generated Skill 02 elicitation draft. It is raw AI output and has not yet been reviewed by the student.

| Field | Value |
|---|---|
| Raw output file | `outputs/raw/02-elicitation.md` |
| Prerequisite baseline | `outputs/reviewed/01-inception.md` |
| Status | Raw elicitation draft |
| Boundary | Elicitation planning and evidence capture only |

This output does not create final functional requirements, non-functional requirements, user stories, use cases, priorities, validation results, or change requests. It prepares questions, evidence categories, and decision points for later review and specification.

## 2. Inputs and Evidence Sources

| Source | Role in this elicitation draft | Use rule |
|---|---|---|
...
| `SRC-02` | Simulated interview record in `inputs/interview-answers.md`. | Use with simulation labels and response status. |
| `SRC-03` | Assumption register in `inputs/assumptions.md`. | Use only as labelled assumptions with `ASM-*` IDs. |
| `outputs/reviewed/01-inception.md` | Reviewed inception baseline. | Primary starting point for stakeholders, open questions, risks, and student decisions. |
| `evaluation/ai-output-review.md` | Review record for Skill 01. | Confirms that unresolved policy questions remain open for elicitation. |

Skill 01 retained these draft open questions for elicitation: `OQ-DRAFT-01`, `OQ-DRAFT-02`, and `OQ-DRAFT-03`.
...
|---|---|---|---|
| Lecturer | Confirmed case stakeholder | Assignment fields, deadline rules, late submissions, resubmissions, grade and feedback publication, grade correction, reporting, and evidence needed for submission disputes. | `SRC-01`; `INT-L-01`; `INT-L-02`; `INT-L-03`; `INT-L-04`; `INT-L-05`; `INT-L-06`; `OQ-01`; `OQ-02`; `OQ-03`; `OQ-06`; `OQ-07`; `OQ-12`; `OQ-DRAFT-03` |
...
| Response ID | Existing status | Skill 02 classification | Remaining gap |
|---|---|---|---|
...
Example blank capture row:

| Answer ID | Stakeholder | Question ID | Source type | Answer text | Status | Date | Reviewer or approver | Related open questions | Specification readiness |
|---|---|---|---|---|---|---|---|---|---|
...
No policy decision is made in this raw output.
...
Skill 03 should not write detailed final requirements until the student reviews this raw elicitation output and decides how to handle unresolved answers.

Minimum readiness criteria:

| Criterion ID | Criterion | Current state |
|---|---|---|
| RC-01 | The student reviews this raw Skill 02 output. | Not done in this raw draft. |
| RC-02 | Every new simulated answer is approved and recorded with answer ID, source label, date, and reviewer. | Not done in this raw draft. |
| RC-03 | Each `OQ-*` and `OQ-DRAFT-*` item is either answered, partially answered with a stated gap, or explicitly deferred. | Not done in this raw draft. |
| RC-04 | Student-approved policies exist for late submission, resubmission, file limits, grade publication, notifications, authentication, reporting, retention, integrations, and measurable NFRs, or those areas are excluded from detailed specification. | Not done in this raw draft. |
| RC-05 | Assumption-based stakeholder topics keep `ASM-*` labels unless validated. | Preserved in this raw draft. |
| RC-06 | No final requirement is written from an unanswered item. | Preserved in this raw draft. |

## 11. Traceability Notes

- Confirmed case facts in this draft trace to `SRC-01` and case-confirmed simulated answers such as `INT-L-01`, `INT-S-01`, `INT-A-01`, and `INT-A-06`.
...
- New elicitation questions use `ELQ-*` identifiers.
- This raw output intentionally avoids final requirements, user stories, use cases, priorities, validation results, and change requests.
